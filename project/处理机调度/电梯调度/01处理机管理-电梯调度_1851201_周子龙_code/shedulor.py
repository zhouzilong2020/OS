...
class Scheduler(threading.Thread):
    def __init__(self, output, insideButton, outsideButton, elevatorNum = 5, maxLevel = 20):
        super().__init__()
        self.elevators = []             # 存放各个电梯
        self.maxLevel = maxLevel        # 最大楼层数
        self.mailbox = queue.Queue(30)  # 进程间通信，用于完成对外部按钮的更新
        self.outsideButton = outsideButton  # 存放外部按钮的GUI控制句柄
        self.output = output
    ...
    def run(self):
        ...
        while 1:
            ...
            if dirc == 1:
                self.outsideButton[level-1]["up"]["state"] = "normal"
            elif dirc == -1:
                self.outsideButton[level-1]["down"]["state"] = "normal"
            else:
                self.outsideButton[level-1]["up"]["state"] = "normal"
                self.outsideButton[level-1]["down"]["state"] = "normal"
    ...
    def insideButton(self, eleID, level):
        self.output.insert(1.0, f"schedule elevator{eleID} to {level}\n")
    ...
    def schedule(self, level, dir):
        ...
        time.sleep(0.2)
        self.output.insert(1.0, f"schedule elevator{elevator.elevatorID} to {level}\n")
